...
// Load & display Bills in Bill View
function loadBills() {
...
function payBill(billIndex, amountNum) {
  ipcRenderer.invoke('get-data').then(data => {
    if (!data || !data.bills) data = { bills: [] };
    ...
    const year = currentMonth.getFullYear();
    const monthIndex = currentMonth.getMonth();
    bill.paymentHistory.push({ year, month: monthIndex, amount: amountNum });

    if ((bill.category === "Loan" || bill.category === "Credit Card") && bill.balance !== undefined) {
      let newBalance = parseFloat(bill.balance) - amountNum;
